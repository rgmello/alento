  ...
  return (
    <div className="min-h-screen px-6 py-24 md:px-12 max-w-5xl mx-auto">
      ...
      <div className="space-y-4">
        {poems.map((poem) => (
          <div key={poem.id} className="flex flex-col md:flex-row items-start md:items-center justify-between p-6 bg-foreground/5 rounded border border-foreground/5 gap-4">
            <div>
              <h2 className="text-xl text-foreground/80 mb-1" style={{ fontFamily: 'var(--font-serif)' }}>{poem.title}</h2>
            ...
            <div className="flex gap-3 mt-4 md:mt-0">
              <Link href={`/admin/edit/${poem.id}`} className="text-sm px-4 py-2 border border-foreground/20 rounded hover:bg-foreground/10 transition-colors">
                Editar
              </Link>
              <form action={deletePoem}>
                <input type="hidden" name="id" value={poem.id} />
                <button type="submit" className="text-sm px-4 py-2 border border-red-500/50 text-red-500 rounded hover:bg-red-500/10 transition-colors" onClick={(e) => {
